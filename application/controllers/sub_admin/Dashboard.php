...
// today code start 
    ...
    $todaySubArr[] = (empty($todayData))?"Today Leads":"Today Leads"; 
    $todaySubArr[] = (empty($todayData))?0:count($todayData); 
    $todayArr[] = $todaySubArr;
    
    $todaySubArr = array();
    $sql ='';
    $sql ="SELECT id FROM `enquery` WHERE created_at BETWEEN '".$tofrom."' AND '".$tonow."' AND `enquery_type` = '1' ";
    $todayfollowupData =  $this->enquery->rawquery($sql);
     
    $todaySubArr[] = (empty($todayfollowupData))?"FollowUp Leads":"FollowUp Leads"; 
    $todaySubArr[] = (empty($todayfollowupData))?0:count($todayfollowupData); 
    $todayArr[] = $todaySubArr;
        
    $todaySubArr = array();
    $sql ='';
    $sql ="SELECT id FROM `enquery` WHERE created_at BETWEEN '".$tofrom."' AND '".$tonow."' AND `spam_Id` = '1' ";
    $todaySpamData =  $this->enquery->rawquery($sql);
     
    $todaySubArr[] = (empty($todaySpamData))?"Spam Leads":"Spam Leads"; 
    $todaySubArr[] = (empty($todaySpamData))?0:count($todaySpamData); 
    $todayArr[] = $todaySubArr;
        
    $todaySubArr = array();
    $sql ='';
    $sql ="SELECT id FROM `enquery` WHERE created_at BETWEEN '".$tofrom."' AND '".$tonow."' AND `enquery_mode` = '1' ";
    $todayHotData =  $this->enquery->rawquery($sql);
     
    $todaySubArr[] = (empty($todayHotData))?"Hot Leads":"Hot Leads"; 
    $todaySubArr[] = (empty($todayHotData))?0:count($todayHotData); 
    $todayArr[] = $todaySubArr;
        
    $todaySubArr = array();
    $sql ='';
    $sql ="SELECT id FROM `enquery` WHERE created_at BETWEEN '".$tofrom."' AND '".$tonow."' AND `enquery_mode` = '2' ";
    $todayColdData =  $this->enquery->rawquery($sql);
     
    $todaySubArr[] = (empty($todayColdData))?"Cold Leads":"Cold Leads"; 
    $todaySubArr[] = (empty($todayColdData))?0:count($todayColdData); 
    $todayArr[] = $todaySubArr;


    $data['jsonTodayData'] = json_encode($todayArr,true);
// today code end 


// pre($data['jsonTodayData']);
// exit;
        $this->global['title'] = 'Techcentrica :Dashboard';
        $this->loadSubAdminViews("sub_admin/dashboard", $this->global, $data , NULL,'sub_admin');
    }
