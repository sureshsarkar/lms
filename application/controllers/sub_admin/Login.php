    ...
    public function loginMe()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (count($result) > 0) {
                ...
                $up['system_ip'] = $_SERVER['REMOTE_ADDR'];
                ...
                $getData =  $this->user_history->finddynamic($w);
                // pre($getData);
                // exit;
                $w = array();
                $w['owner_id']   = $result[0]->id;
                ...
                $w['created_at'] =date('Y-m-d');
                $w['parent_id']   = 0;
                $w['system_ip']  = $_SERVER['REMOTE_ADDR'];

                if(empty($getData)){
                    $loginId =  $this->user_history->save($w);
                }else{
                    $w = array();
                    $w['owner_id'] = $result[0]->id;
                    $w['created_at'] = date("Y-m-d");
                    ...
                    $w['limit']    = 1;
                    $w['field']    = "id,created_at,";
                    $todayFirst =  $this->user_history->finddynamic($w);
// pre($todayFirst);
// exit;
                    $w = array();
                    $w['owner_id']   = $result[0]->id;
                ...
                foreach ($result as $res) {
                    $sessionArray = ['userId' => $res->id, 'role' => $res->role, 'email' => $res->email, 'phone' => $res->phone, 'name' => $res->name, 'isLoggedInSubAdmin' => true, 'loginId'=>$loginId];
    ...
    function logout()
    {
        ...
        $w['last_activity'] =date('Y-m-d H:i:s');
        ...
        $w = array();
        $w['id']         = $_SESSION['loginId'];
        $w['logout_at']  =date("Y-m-d H:i:s");
        $w['updated_at'] =date("Y-m-d H:i:s");
        $w['logout_ip'] = $_SERVER['REMOTE_ADDR'];
