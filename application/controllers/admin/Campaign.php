...
class Campaign extends BaseController
{
    ...
    function index()
    {
        ...
        $data['title'] = 'Techcentrica :Campaign';

        $deviceType = getDevice();
        if(isset($deviceType) && $deviceType == "mobile"){
            $this->load->view('admin/campaign/managemobile', $data); 
        }else{
    ...
    public function add()
    {
        ...
        $this->load->view('admin/campaign/add', $data); 
           ...
           // upload image end
                    ...
                    $result  = $this->campaign->save($updateData);
                        ...
                        if($result >0){
                            ...
                            redirect('admin/campaign/edit/'.$id);
                        }
            }
        }
    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $currentObj) {
            ...
            if($fileExtension == "docx"){
                ...
                $fileType = '<img src="'.base_url().'assets/images/pdf.png" style="width:48px;" alt="">';
            ...
            $row[] = '<a href="'.base_url('admin/campaign/view/').$currentObj->id.'" class="nameClass">'.$fileType.'</a>';
            $row[] ='<a href="'.base_url('admin/campaign/view/').$currentObj->id.'" class="nameClass">'.$currentObj->filename.'</a>';
            $row[] =$currentObj->sname;
            $row[] = $currentObj->updated_at;
            $row[] = $date_at;
             $row[] ='<a class="" href="' .base_url() .'admin/campaign/edit/' .$currentObj->id .'" title="Edit campaign Base" ><button type="button" class="btn-gradient-primary"><i class="bi bi-pencil-square"></i> Edit</button></a>&nbsp;&nbsp;<button type="button" class="btn-gradient-primary sendEmail" imgUrl="'.$currentObj->image.'" filename="'.$currentObj->filename.'" subject="'.$currentObj->subject.'"> <i class="bi bi-envelope-check-fill"></i> Mail</button>';
...
// delete
    ...
    function status($field, $id)
    {
        ...
        //     $subject = "Test";
        //     $message = "mdfgjidrj";
        //    echo $r =  $this->send_email('suresh.s@techcentrica.com', $subject, $message);
        //    exit();
