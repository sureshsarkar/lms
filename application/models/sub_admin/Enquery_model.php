...
class Enquery_model extends Base_model

{
    public $table = "enquery";

    var $column_order = array('sa.id','enquery.name','enquery.email','enquery.phone','enquery.status','enquery.created_at','sa.name'); //set column field database for datatable orderable

    var $column_search = array('sa.id','enquery.name','enquery.email','enquery.phone','enquery.status','enquery.created_at','sa.name');//set column field database for datatable searchable 
    ...
    {     
        ...
        $this->db->select('enquery.*,sa.id as sid, sa.name as sname,sa.last_activity as last_activity,sa.picture as picture');
    ...
    public function jointTables(){
        ...
        $this->db->join('sub_admin as sa ',' sa.id = enquery.lead_owner','left');

        // $this->db->join('sub_category as sc ',' sc.id = sub_category_id');
    ...
    public function getAction(){
        ...
        $this->db->where('spam_Id','0'); // moved to spam

        if(isset($_SESSION['userId']) && $_SESSION['userId'] !== '' && isset($_SESSION['isLoggedInSubAdmin'])){
        ...
        if(isset($_GET['daterange']) && $_GET['daterange'] == 'daterangefilter'){
            ...
            $this->db->where('enquery.created_at >=', $from_date);
            $this->db->where('enquery.created_at <=', $to_date);
        }
